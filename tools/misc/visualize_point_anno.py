...
def parse_args():
    ...
    parser.add_argument('--result', help='results file in pickle format',
                        default="tec_voxel01_center_res.pkl")
...
def main():
    ...
    if getattr(dataset, 'show', None) is not None:
        ...
        if eval_pipeline:
            dataset.show_point_anno(results, args.show_dir, pipeline=eval_pipeline, save_show=args.save_pic)
        else:
            dataset.show_point_anno(results, args.show_dir, save_show=args.save_pic)  # use default pipeline
    else:
