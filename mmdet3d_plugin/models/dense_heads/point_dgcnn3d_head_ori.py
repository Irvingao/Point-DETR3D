...
class PointDGCNN3DHead(DETRHead):
    ...
    def forward(self, mlvl_feats,point_coord,labels):
        ...
        # pointencoder = PointEncoder(10,128)
        # query_embeds = pointencoder.forward(point_coord,labels,self.pc_range)
        ...
        # [6,1,19,256]
        # hs = hs.permute(0, 2, 1, 3)  # 转到前面transformer中去做
        ...
        for idx in range(len(hs)):
            outputs_classes = []
            outputs_coords = []
            for lvl in range(hs[idx].shape[0]):
                if lvl == 0:
                    reference = init_reference[idx]
                else:
                    reference = inter_references[idx][lvl - 1]
                reference = inverse_sigmoid(reference)
                outputs_class = self.cls_branches[lvl](hs[idx][lvl])
                tmp = self.reg_branches[lvl](hs[idx][lvl])
            ...
            all_outputs_coords.append(outputs_coords)
            all_outputs_classes.append(outputs_classes)
        ...
        if self.as_two_stage:
            ...
            outs = {
                'all_cls_scores': all_outputs_classes,
                'all_bbox_preds': all_outputs_coords,
                'enc_cls_scores': None,
                'enc_bbox_preds': None,
            }
        return outs
    ...
    def _get_target_single(self,
        ...
        # 根本不需要算分类的loss,这里的label直接给成gt
        # labels = gt_labels
        # label_weights = gt_bboxes.new_ones(num_bboxes)
        #
        # bbox_targets = gt_bboxes
        #
        # bbox_weights = torch.ones_like(bbox_pred)
        # pos_inds = [i for i in range(num_bboxes)]
        # pos_inds = torch.tensor(pos_inds,device=bbox_targets.device)
        # neg_inds = []
        # neg_inds = torch.tensor(neg_inds,device=bbox_targets.device)
    ...
    def loss_single(self,
        ...
        # loss_cls = torch.tensor([0.0],device=bbox_targets.device)
    ...
    def loss(self,
        ...
        num_dec_layers = all_cls_scores[0].size(0)
        bs = len(all_cls_scores)
        # --------------
        ...
        for layer_id in range(num_dec_layers):
            ...
            new_bbox_pred = []
            for idx in range(bs):
                new_cls_scores.append([])
                new_bbox_pred.append([])
            all_new_cls_scores.append(new_cls_scores)
            all_new_bbox_pred.append(new_bbox_pred)


        for idx in range(bs):
            for layer_id in range(num_dec_layers):
                all_new_cls_scores[layer_id][idx] = all_cls_scores[idx][layer_id]
                all_new_bbox_pred[layer_id][idx] = all_bbox_preds[idx][layer_id]
        ...
        losses_cls, losses_bbox = multi_apply(
            self.loss_single, all_new_cls_scores, all_new_bbox_pred,
