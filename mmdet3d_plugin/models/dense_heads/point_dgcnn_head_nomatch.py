...
class PointDGCNN3DHead_Nomatch(DETRHead):
    ...
    def __init__(self,
        ...
        super(PointDGCNN3DHead_Nomatch, self).__init__(
    ...
    def forward(self, mlvl_feats,point_coord,labels):
        ...
        pointencoder = PointEncoderV2(10, 128)
        ...
        for lvl in range(hs.shape[0]):
            ...
            outputs_class = torch.zeros_like(tmp)
            for batch_id in range(len(labels)):
                for idx in range(len(labels[batch_id])):
                    outputs_class[batch_id][idx][labels[batch_id][idx].type(torch.long)] = 1
    ...
    def new_get_targets(self,
        ...
        # ------------------------------------------------------------
        # 因为这里全都是正样本，所以直接将gt赋值给相应的target
        labels_list = gt_labels_list
        bbox_targets_list = gt_bboxes_list
        label_weights_list = []
        bbox_weights_list = []
        num_total_pos = 0
        num_total_neg = 0
        for batch_id in range(len(gt_bboxes_list)):
            label_weights_list.append(gt_bboxes_list[batch_id].new_ones(gt_bboxes_list[batch_id].shape[0]))
            bbox_weights_list.append(gt_bboxes_list[batch_id].new_ones(bbox_preds_list[batch_id].shape))
            num_total_pos = num_total_pos + len(bbox_preds_list[batch_id])
    ...
    def new_loss_single(self,
        ...
        # cls_scores:[300*2,10]  labels:[300*2] 这里面预测和gt都是一一对应的
        loss_cls = self.loss_cls(
            new_cls_scores, labels.long(), label_weights, avg_factor=cls_avg_factor)
