...
class DeformCrossAttn(BaseModule):
    ...
    def __init__(self,
                 ...
                 fix_offset=False,
                 depth_encode=False):
        super(DeformCrossAttn, self).__init__(init_cfg)
        if embed_dims % num_heads != 0:
            raise ValueError(f'embed_dims must be divisible by num_heads, '
                             f'but got {embed_dims} and {num_heads}')
        dim_per_head = embed_dims // num_heads
    ...
    def forward(self,
        ...
        mask = mask.view(B, num_cam, num_query, 1)
        ...
        # reference_points_cam = reference_points_cam.unsqueeze(1).repeat(1, 1, len(mlvl_feats), 1)
        reference_points_cam = reference_points_cam.view(B * self.num_cams, num_query, 1, 2).repeat(1, 1, self.num_levels, 1)
        ...
        sampling_offsets = self.deform_sampling_offsets(query_cam).view(
            bs * self.num_cams, num_query, self.num_heads, self.num_levels, self.num_points, 2)
        ...
        cam_attention_weights = cam_attention_weights.sigmoid() * mask
