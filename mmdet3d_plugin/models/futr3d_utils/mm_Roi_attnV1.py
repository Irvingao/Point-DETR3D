...
class MMRoiAttn(ImgRoiCrossAttn):
        ...
        **kwargs allow passing a more general data flow when combining
        ...
        if self.img_roi:
            ...
            if self.drop_query:
                ...
                query, query_pos, query_mapping = self.drop_invalid_query(query, query_pos, drop_idx)
            else:
                drop_cam_mask, drop_idx = self.rand_cam_mask(cam_mask)
                
            # 去除中心点没有投在image上的无效点
            box2d_corners, box2d_cetners, vaild_cam_mask = \
            ...
            img_roi_feats, roi_pts = self.box2imgRoi(
                ...
                box2d_cetners, vaild_cam_mask, kwargs['img_metas'])
        ...
        if self.drop_query:
            if drop_idx.size(0) > 0:
                newout = torch.zeros_like(identity)
                newout[query_mapping>-1] += self.dropout_layer(self.proj_drop(out))
                newout[drop_idx] = identity[drop_idx]
                return newout
            # newout = identity[query_mapping>-1] + self.dropout_layer(self.proj_drop(out))
    ...
    def drop_invalid_query(self, query, query_pos, drop_idx):
        num_query, bs, dim = query.size()
        if drop_idx.size(0) == 0:
            return query, query_pos, torch.arange(query.size(0))
        valid_mask = torch.full([num_query], True)
        valid_mask[drop_idx] = False
        # filter
        vaild_query = query[valid_mask]
        vaild_query_pos = query_pos[valid_mask]
        
        # valid_mapping = torch.zeros(num_query, dtype=torch.long)
        # valid_mapping[drop_idx] = -1
        # valid_mapping[valid_mapping == 0] = torch.arange(vaild_query.size(0))
        
        valid_mapping = torch.arange(num_query)
        valid_mapping[drop_idx] = -1
        
        return vaild_query, vaild_query_pos, valid_mapping

    def drop_invalid_bev_center(self, bev2d_corners, drop_idx):
        if drop_idx.size(0) == 0:
            return bev2d_corners
        bs, num_valid, dim = bev2d_corners.size()
        valid_mask = torch.full([num_valid], True)
        valid_mask[drop_idx] = False
        
        vaild_bev2d_corners = bev2d_corners[:, valid_mask]

        return vaild_bev2d_corners
