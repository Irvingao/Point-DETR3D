...
class Deform3DCamCrossAttnV13(BaseModule):
    ...
    def __init__(self,
        ...
        super(Deform3DCamCrossAttnV13, self).__init__(init_cfg)
        ...
        self.with_main_pts = with_main_pts
        ...
        if self.with_main_pts:
            self.deform_sampling_offsets = nn.Linear(
                embed_dims, num_heads * 1 * (num_points-1) * 3) # √
        else:
            self.deform_sampling_offsets = nn.Linear(
    ...
    def forward(self,
        ...
        sampling_offsets = self.deform_sampling_offsets(query).view(
            bs, num_query, self.num_heads, 1, num_points, 3).repeat(1, 1, 1, self.num_levels, 1, 1)
        # V10 without normalize_offset
        # sampling_offsets = self.normalize_offset(sampling_offsets)
        
        # V11 without offset, repeat all
        # reference_points = reference_points.view(bs, num_query, 1, 1, 1, 3) + sampling_offsets
        # reference_points = reference_points.view(bs, num_query, 1, 1, 1, 3).repeat(
        #                         1, 1, self.num_heads, self.num_levels, self.num_points, 1)
        
        # V12 normalize_offsetV2 
        sampling_offsets = self.normalize_offsetV2(sampling_offsets)
        
        if self.with_main_pts:
            non_offset = sampling_offsets.new_zeros((bs, num_query, self.num_heads, self.num_levels, 1, 3))
            sampling_offsets = torch.cat([sampling_offsets, non_offset], dim=-2)
        reference_points = reference_points.view(bs, num_query, 1, 1, 1, 3) + sampling_offsets
        ...
        for i in range(len(value)):
            if value[i].size(0) == self.num_cams:
                value[i] = value[i].permute(1,0,2,3,4)  # add reverse 5.8晚上，发现不更改顺序掉点
    ...
    def normalize_offset(self, sampling_offsets):
        ...
        sampling_offsets = sampling_offsets.sigmoid()
