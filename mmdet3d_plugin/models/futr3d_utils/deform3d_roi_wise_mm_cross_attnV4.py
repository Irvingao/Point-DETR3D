...
class Deform3DRoIWiseMultiModalCrossAttnV4(Deform3DMultiModalCrossAttn):
    ...
    def __init__(self,
        ...
        self.cam_deform = cam_deform
        self.img_norm = img_norm
        
        if not self.cam_deform:
            self.num_cam_points = 1
            self.cam_attention_weights = nn.Linear(self.embed_dims,
                self.num_heads*self.num_levels*self.num_cam_points)
            self.deform_cam_sampling_offsets = None
        ...
        self.with_cam_fusion = with_cam_fusion
    ...
    def forward(self,
        ...
        if self.use_Cam:
            ...
            # ##################################################################
            if self.cam_deform:
                # align with `mmcv/ops/multi_scale_deform_attn.py`
                ...
                cam_attention_weights = cam_attention_weights.softmax(-1)
                
                cam_attention_weights = cam_attention_weights.view(bs * self.num_cams, 
                    num_query, self.num_heads, self.num_levels, self.num_cam_points)
            else:
                cam_attention_weights = self.cam_attention_weights(query_cam).view(
                    bs * self.num_cams, num_query, self.num_heads, self.num_cam_points, self.num_levels).sigmoid()
                cam_attention_weights = cam_attention_weights.permute(0,1,2,4,3)
            ...
            if self.cam_deform:
                ...
                if self.offset_2d:
                    # without any normalization for cam_sampling_offsets
                    reference_points_2d = reference_points[:,:,None,None,None,:2] + cam_sampling_offsets[...,:2]
                    reference_points_h = reference_points[:,:,None,None,None,2:3] + torch.zeros_like(cam_sampling_offsets[...,0:1])
                    reference_points = torch.cat([reference_points_2d, reference_points_h],dim=-1)
                else:
                    reference_points = reference_points[:,:,None,None,None,:] + cam_sampling_offsets
                # ##################################################################
            else:
                reference_points = reference_points.view(bs, num_query,1,1,1,3).repeat(1, 1, self.num_heads, self.num_levels, 1, 1)
            ...
            cam_attention_weights = cam_attention_weights * mask
            ...
            if self.img_roi:
                ...
                if self.img_roi_offset:
                    if self.img_norm:
                        cam_roi_sampling_offsets = self.roi_cam_sampling_offsets(query).view(
                            bs, num_query, self.num_heads, self.num_levels, self.num_img_roi_pts, 2)
                        if reference_points_cam_roi.shape[-1] == 2:
                            cam_offset_normalizer = torch.stack(
                                [cam_spatial_shapes[..., 1], cam_spatial_shapes[..., 0]], -1)
                            reference_points_cam_roi = reference_points_cam_roi \
                                + cam_roi_sampling_offsets \
                                / cam_offset_normalizer[None, None, None, :, None, :]
                        else:
                            raise ValueError(
                                f'Last dim of reference_points must be'
                                f' 2, but get {reference_points.shape[-1]} instead.')
