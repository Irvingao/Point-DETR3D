...
def _draw_points(points,
    ...
    if mode == 'xyz':
        ...
        if not ((points_colors >= 0.0) & (points_colors <= 1.0)).all():
            points_colors /= 255.0
            # print("!!!!!!!!!!")
        # numpy.array [N,3] -> list [ array [3] ]
        points_colors = [points_colors[i] for i in range(points_colors.shape[0])] 
        # print(points_colors[:3])
        # print(len(points_colors))
    else:
...
class Visualizer(object):
    ...
    def __init__(self,
        ...
        # draw background
        self.o3d_visualizer.get_render_option().background_color = background_color # RGB(0,0,0)表黑色
