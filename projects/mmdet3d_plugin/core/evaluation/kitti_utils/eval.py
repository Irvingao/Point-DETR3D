...
# TODO 这里由于nuscene要用，把bbox部分都注释掉了
...
def _prepare_data(gt_annos, dt_annos, current_class, difficulty):
    ...
    for i in range(len(gt_annos)):
        ...
        # print("dt_annos[i]['alpha'][..., np.newaxis].shape:", dt_annos[i]['alpha'][..., np.newaxis].shape)
        # print("dt_annos[i]['score'][..., np.newaxis].shape:", dt_annos[i]['score'][..., np.newaxis].shape)
...
def do_eval(gt_annos,
    ...
    if 'bbox' in eval_types:
        ...
        if 'aos' in eval_types:
            mAP_aos = get_mAP_R40(ret['orientation'])

    mAP_bev = None
    if 'bev' in eval_types:
        ret = eval_class(gt_annos, dt_annos, current_classes, difficultys, 1,
                         min_overlaps)
        mAP_bev = get_mAP_R40(ret['precision'])

    mAP_3d = None
    if '3d' in eval_types:
        ret = eval_class(gt_annos, dt_annos, current_classes, difficultys, 2,
                         min_overlaps)
        mAP_3d = get_mAP_R40(ret['precision'])
...
def kitti_eval(gt_annos,
    ...
    for anno in gt_annos:
        # print("type(anno['alpha']):", type(anno['alpha']))
        print("anno['alpha'].shape:", anno['alpha'].shape)
        print("anno:", anno)
        if anno['alpha'][0] != -10:
            valid_alpha_gt = True
            break
