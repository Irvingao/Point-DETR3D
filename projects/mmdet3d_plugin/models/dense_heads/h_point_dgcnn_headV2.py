...
from mmdet.core.bbox.match_costs import match_cost

@HEADS.register_module()
class HPointDGCNN3DHeadV2(DETRHead):
    ...
    def __init__(self,
        ...
        super(HPointDGCNN3DHeadV2, self).__init__(
    ...
    def forward(self, mlvl_feats,point_coord,labels):
        ...
        pointencoder = Hybrid_PointEncoder(10, 128)
        ...
        # 计算num_queries_one2one

        positive_num = [point_coord[idx].size()[0] for idx in range(len(point_coord))]
        num_queries_one2one = max(positive_num)
        #-------
        all_cls_scores_one2one = outputs_classes[:, :, 0: num_queries_one2one, :]
        all_bbox_preds_one2one = outputs_coords[:, :, 0: num_queries_one2one, :]
        all_cls_scores_one2many = outputs_classes[:, :, num_queries_one2one:, :]
        all_bbox_preds_one2many = outputs_coords[:, :, num_queries_one2one:, :]
    ...
    def loss(self,
        ...
        losses_cls_one2many, losses_bbox_one2many = multi_apply(
            self.new_loss_single, all_cls_scores_one2many, all_bbox_preds_one2many,
    ...
    def get_bboxes(self, preds_dicts, img_metas, rescale=False):
        ...
        for i in range(num_samples):
            preds = preds_dicts[i]
            bboxes = preds['bboxes']
            # 再从gravity center转换为和gt一样的 地面中心
