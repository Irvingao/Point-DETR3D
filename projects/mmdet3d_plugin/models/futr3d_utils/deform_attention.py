...
# /home/gaohz/.local/lib/python3.9/site-packages/mmcv/ops/multi_scale_deform_attn.py
import math
import warnings
import numpy as np
import torch
...
class DeformFUTR3DCrossAtten(BaseModule):
    ...
    def __init__(self,
                 ...
                 bev_norm=False,
                 pc_norm=True,
                 with_direct_point=True,
                 norm_cfg=None,
                 init_cfg=None,
                 **kwargs):
        super(DeformFUTR3DCrossAtten, self).__init__(init_cfg)
        ...
        self.with_direct_point = with_direct_point
        
        if self.bev_norm:
            self.spatial_shapes = [128//pow(2,i) for i in range(4)]
            # self.spatial_shape = [[128,128], [64,64],[32,32],[16,16]]
                
        if self.use_Cam:
            if self.deform:
                # cam deformable 
                if self.with_direct_point:
                    self.num_cam_points = offset_points + num_points
                else:
                    self.num_cam_points = offset_points
            ...
            if self.deform:
                ...
                if self.bev_norm:
                    spatial_shapes = torch.stack([reference_points.new_tensor(
                        [spatial_shape, spatial_shape]) for spatial_shape in self.spatial_shapes])
                    # spatial_shapes = torch.stack([reference_points.new_tensor(
                        # [feat.shape[-2], feat.shape[-1]]) for feat in pts_feats])
                    cam_sampling_offsets = self.normalize_offset_bev(cam_sampling_offsets, spatial_shapes)
                elif self.pc_norm:
                    pass
                else:
                    cam_sampling_offsets = self.normalize_offset(cam_sampling_offsets)

                if self.with_direct_point:
                    # add direct point
                    cam_sampling_offsets = torch.cat((cam_sampling_offsets, torch.zeros_like(
                        cam_sampling_offsets[..., :1, :])), -2)
                # add zero-offset height.
                cam_sampling_offsets = torch.cat((cam_sampling_offsets, torch.zeros_like(
                    cam_sampling_offsets[..., :1])), -1)
                ...
                if self.softmax == 'mlk':
                    ...
                    img_attention_weights = self.attention_weights(query).view(
                        bs, 1, num_query, self.num_cams, self.num_heads, self.num_cam_points, self.num_levels).sigmoid()
            else:
                # (B, 1, num_query, num_cams, num_points, num_levels)
                img_attention_weights = self.attention_weights(query).view(
                    bs, 1, num_query, self.num_cams, self.num_cam_points, self.num_levels).sigmoid()
                reference_points_cam = reference_points
            # output (B, C, num_query, num_cam, num_points, len(lvl_feats))
            reference_points_3d, img_output, mask = feature_sampling(
                img_feats, reference_points_cam, self.pc_range, kwargs['img_metas'], sampling_offsets=sampling_offsets)
    ...
    def normalize_offset(self, sampling_offsets):
    ...
    def normalize_offset_bev(self, sampling_offsets, spatial_shapes):
        offset_normalizer = torch.stack(
        ...
        # sampling_offsets = sampling_offsets.unsqueeze(1).repeat(1,self.num_cams,1,1,1,1,1) \
        sampling_offsets = sampling_offsets \
            / offset_normalizer[None, None, None, :, None, :]
        return sampling_offsets
    

def feature_sampling(mlvl_feats, reference_points, pc_range, img_metas, sampling_offsets=None):
    lidar2img = []
    ...
    if sampling_offsets is not None:
        reference_points = reference_points[:,:,None,None,None,:] + sampling_offsets
    deform = False
    ...
    if deform:
        # reference_points (B, num_queries, 4) | (B, num_queries, num_h, num_lvl, num_pts, 4)
        reference_points = torch.cat((reference_points, 
            torch.ones_like(reference_points[..., :1]).repeat(1,1,1,1,1,4-pts_dim)), -1)

        num_cam = lidar2img.size(1)
        # ref_point change to (B, num_cam, num_query, 4, 1) | (B, num_cam, num_queries, num_h, num_lvl, num_pts, 4, 1)
        reference_points = reference_points.view(B, 1, num_query, num_h, num_lvl, num_pts, 4) \
            .repeat(1, num_cam, 1, 1, 1, 1, 1).unsqueeze(-1)
        # lidar2img chaneg to (B, num_cam, num_query, 4, 4) | (B, num_cam, num_queries, num_h, num_lvl, num_pts, 4, 4)
        lidar2img = lidar2img.view(B, num_cam, 1, 4, 4).repeat(1, 1, num_query*num_h*num_lvl*num_pts, 1, 1) \
            .view(B, num_cam, num_query, num_h, num_lvl, num_pts, 4, 4)
        # ref_point_cam change to (B, num_cam, num_query, 4) | (B, num_cam, num_queries, num_h, num_lvl, num_pts, 4)
        reference_points_cam = torch.matmul(lidar2img, reference_points).squeeze(-1)    # torch.Size([1, 6, 26, 8, 4, 4, 4])
        eps = 1e-5
        ...
        mask = (mask & (reference_points_cam[..., 0:1] > -1.0)
                    ...
                    & (reference_points_cam[..., 1:2] < 1.0))
        # mask: torch.Size([1, 6, 26, 8, 4, 4, 1])
        # mask shape (B, 1, num_query, num_cam, 1, 1) | (B, 1, num_queries, num_cam, num_h, num_lvl, num_pts, 1)
        # mask = mask.view(B, num_cam, 1, num_query, 1, 1).permute(0, 2, 3, 1, 4, 5) 
        mask = mask.view(B, num_cam, 1, num_query, num_h, num_lvl, num_pts).permute(0, 2, 3, 1, 4, 6, 5) 
        # mask: torch.Size([1, 1, 26, 6, 8, 4, 4, 1, 1])
        mask = torch.nan_to_num(mask)
        sampled_feats = []
        num_points = 1
        ...
        # sampled_feats = sampled_feats.view(B, C, num_query, num_cam, num_h, num_pts, len(mlvl_feats))
