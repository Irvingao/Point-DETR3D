...
class Deform3DRoIWiseMultiModalCrossAttn(Deform3DMultiModalCrossAttn):
    ...
    def __init__(self,
        ...
        if self.img_roi:
            self.cam_attention_weights = nn.Linear(self.embed_dims,
                self.num_heads*self.num_levels*(self.num_cam_points+self.num_roi_pts))
        if self.pts_roi:
            self.lid_attention_weights = nn.Linear(self.embed_dims,
                self.num_heads*self.num_levels*(self.num_lid_points+self.num_roi_pts))
        ...
        if self.use_LiDAR:
            ...
            if self.pts_roi:
                lid_attention_weights = self.lid_attention_weights(query).view(
                    bs, num_query, self.num_heads, self.num_levels * (self.num_lid_points+self.num_roi_pts))
        ...
        if self.use_Cam:
            ...
            if self.img_roi:
                cam_attention_weights = self.cam_attention_weights(query_cam).view(
                    bs * self.num_cams, num_query, self.num_heads, self.num_levels * (self.num_cam_points+self.num_roi_pts))
            else:
                cam_attention_weights = self.cam_attention_weights(query_cam).view(
            ...
            if self.img_roi:
                ...
                roi_mask = roi_mask.unsqueeze(-1).repeat(1,1,self.num_heads, self.num_levels,self.num_roi_pts)
                # reference_points_cam_roi: torch.Size([6, 26, 8, 4, 49, 2])
                # roi_mask: torch.Size([6, 26, 8, 49])
                
                reference_points_cam = torch.cat([reference_points_cam, reference_points_cam_roi], dim=-2)
                mask = torch.cat([mask, roi_mask], dim=-1)
                
                mask = mask.view(bs * self.num_cams, num_query, self.num_heads, 
                    self.num_levels * (self.num_cam_points+self.num_roi_pts))
    ...
    def box2bevLoc(self, gt_bboxes_3d, img_metas, pc_range, device_tensor=None):
        ...
        if self.rotate_bev_grid:
            ...
            grid = grid.view(num_box, self.num_roi_pts, 2)
    ...
    def box2imgRoiLoc(self, gt_bboxes_3d, mask, lidar2img, img_metas, pc_range):
        ...
        roi_mask = roi_mask[...,-1:,:]    # .repeat(1,1,self.num_heads, self.num_roi_pts) # 
