...
class MMPointObjDGCNNV4(MVXTwoStageDetector):
    ...
    def simple_test_mm(self, pts_feats, img_metas, img_feats=None, rescale=False,**kwargs):
        ...
        outs = self.pts_bbox_head(pts_feats, img_feats, img_metas=img_metas, 
                    ...
                    gt_bboxes_3d=None)
                    # gt_bboxes_3d=kwargs['gt_bboxes_3d'][0])    # 可视化only!!!!
        ...
        # #----------------------------------------------------------
        if outs['all_bbox_preds'].shape[1] != 1:    # [num_dec, bs, num_obj, dim]
        ...
        # 去除GT为0的所有预测值
        if kwargs['labels'][0][0].size(0) == 0:
            empty_out = outs.copy()
            num_dec, bs, num_obj, dim = outs['all_cls_scores'].shape
            empty_out['all_cls_scores'] = outs['all_cls_scores'].new_tensor([]).reshape(num_dec, bs, 0, dim)
            empty_out['all_bbox_preds'] = outs['all_bbox_preds'].new_tensor([]).reshape(num_dec, bs, 0, dim)
            bbox_list = self.pts_bbox_head.get_bboxes(
                empty_out, img_metas, rescale=rescale)
            # print(f"{kwargs['labels'][0][0]}")
            # print(f"{bbox_list}")
