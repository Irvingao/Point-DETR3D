...
from projects.mmdet3d_plugin.models.depth_utils.view_transformer import DepthNet

@DETECTORS.register_module()
class MMPointObjDGCNNDepth(MVXTwoStageDetector):
    ...
    def __init__(self,
        ...
        if self.use_Cam and self.use_grid_mask:
            self.grid_mask = GridMask(True, True, rotate=1, offset=False, ratio=0.5, mode=1, prob=0.7)
        self.depth_net = DepthNet(**depthnet_cfg)
    ...
    def extract_depth_feat(self, input):
    ...
    def extract_feat(self, points, img, img_metas):
        ...
        if not self.use_LiDAR:
            points = None
        img_feats, depth = self.extract_img_feat(img, img_metas)  # bevdepth
        # img_feats = self.extract_img_feat(img, img_metas) # detr3d
    ...
    def extract_img_feat(self, imgs, img_metas):
        ...
        # get bevdepth img
        img, rots, trans, intrins, post_rots, post_trans, bda = \
                self.prepare_inputs(imgs)
        ...
        depth = self.extract_depth_feat(
            [x, rot, tran, intrin, post_rot, post_tran, bda, mlp_input])
        
        return img_feats_reshaped, depth
    ...
    def forward_train(self,
        ...
        img_feats, pts_feats, _ = self.extract_feat(
            points, img=img_inputs, img_metas=img_metas)    # depth
        losses = dict()
        # multi-modal
        losses_mm = self.forward_mdfs_train(pts_feats, img_feats, 
                                            point_coord, labels, 
                                            gt_bboxes_3d, gt_labels_3d, 
                                            img_metas, gt_bboxes_ignore)
        losses.update(losses_mm)
    ...
    def forward_mdfs_train(self,
        ...
        """
        outs = self.pts_bbox_head(pts_feats, img_feats, img_metas=img_metas, 
                                  point_coord=point_coord, labels=labels)
        # box_dim 9;
    ...
    def simple_test_mm(self, pts_feats, img_metas, img_feats=None, rescale=False,**kwargs):
        """Test function of point cloud branch."""
        # outs = self.pts_bbox_head(pts_feats , kwargs['point_coord'][0],kwargs['labels'][0][0])
        outs = self.pts_bbox_head(pts_feats, img_feats, img_metas=img_metas, 
                    point_coord=kwargs['point_coord'][0], labels=kwargs['labels'][0][0])
        # # ---------------------将outs的多余的预测结果去除掉----------
        # positive_num = kwargs['point_coord'][0][0].size()[0]
        # outs['all_cls_scores'] = outs['all_cls_scores'][:,:,:positive_num,:]
        # outs['all_bbox_preds'] = outs['all_bbox_preds'][:, :, :positive_num, :]
        # #----------------------------------------------------------
        if outs['all_bbox_preds'].shape[1] != 1:
            outs['all_cls_scores'] = outs['all_cls_scores'][:,0,...].unsqueeze(1)
            outs['all_bbox_preds'] = outs['all_bbox_preds'][:, 0, ...].unsqueeze(1)
        bbox_list = self.pts_bbox_head.get_bboxes(
            outs, img_metas, rescale=rescale)
        bbox_results = [
            # bbox3d2result将预测结果转到cpu上 并转成ndarray的形式
    ...
    def simple_test_pts(self, x, img_metas, rescale=False,**kwargs):
        """Test function of point cloud branch."""
        outs = self.pts_bbox_head(x,kwargs['point_coord'][0],kwargs['labels'][0][0])
        # # ---------------------将outs的多余的预测结果去除掉----------
        # positive_num = kwargs['point_coord'][0][0].size()[0]
        # outs['all_cls_scores'] = outs['all_cls_scores'][:,:,:positive_num,:]
        # outs['all_bbox_preds'] = outs['all_bbox_preds'][:, :, :positive_num, :]
    ...
    def simple_test(self, points, img_metas, img=None, rescale=False,**kwargs):
        ...
        if self.point_init:
            # multi-modal
            bbox_pts = self.simple_test_mm(
                pts_feats, img_metas, img_feats=img_feats, rescale=rescale,**kwargs)
            for result_dict, pts_bbox in zip(bbox_list, bbox_pts):
                result_dict['pts_bbox'] = pts_bbox
        else:
            if pts_feats and self.with_pts_bbox:
                bbox_pts = self.simple_test_pts(
                    pts_feats, img_metas, rescale=rescale,**kwargs)
                for result_dict, pts_bbox in zip(bbox_list, bbox_pts):
                    result_dict['pts_bbox'] = pts_bbox
    ...
    def show_results(self, data, result, out_dir):
        ...
        for batch_id in range(len(result)):
            ...
            # 画出gt
            gt_bbox =data['img_metas'][0].data[0][0]['gt_boxes']
            show_result(points, gt_bbox, pred_bboxes, out_dir, file_name)
